...
export default function TransferAmount({ params }: { params: { id: string } | Promise<{ id: string }> }) {
  ...
  const handleSchedule = () => {
    ...
    if (amount > 0 && amount <= account.balance) {
      dispatch(setAmount(amount))
      router.push(`/transfer/${id}/schedule`)
    }
  }
  ...
  return (
    <div className="min-h-screen bg-white" data-testid="transfer-amount-page">
      ...
      <div className="fixed bottom-20 left-0 right-0 p-4 bg-white border-t z-50" data-testid="action-buttons-section">
        <div className="space-y-3">
          ...
          <button
            ...
            disabled={!inputAmount || Number(inputAmount) <= 0}
            className="w-full bg-gray-100 text-gray-800 py-4 rounded-lg font-semibold disabled:bg-gray-200 disabled:text-gray-400 border"
            aria-label="Programar para más tarde"
